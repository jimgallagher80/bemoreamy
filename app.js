(() => {
  ...
  // Set-off and finish times per leg (source: route timing sheet)
  ...
  function getLegTiming(leg) {
    ...
    if (LEG_TIMES && LEG_TIMES[key]) {
      return LEG_TIMES[key];
    }
  ...
  function renderSignupLink(leg) {
    ...
    // Always render as a link so the user can tap to see names (even if zero).
    return `<a href="#" class="signupLink" data-leg="${key}">${count} signed up</a>`;
  }

  function openSignupModal(legKey) {
    const modal = document.getElementById("signupModal");
    const title = document.getElementById("signupModalTitle");
    const body = document.getElementById("signupModalBody");
    ...
    if (names.length === 0) {
      body.innerHTML = `<p style="margin:0; color: var(--muted);">No approved sign-ups yet.</p>`;
    } else {
      const items = names
        .map(n => `${safe(n.first_name)} ${safe(n.last_name)}`.trim())
        .map(t => `<li>${t}</li>`)
        .join("");
      body.innerHTML = `<ul style="margin:0; padding-left:18px; line-height:1.5;">${items}</ul>`;
    }
  ...
  function renderDetails(props) {
    ...
    detailsGrid.innerHTML = `
      ...
      <div class="row"><div class="label">Set off time</div><div class="value">${safe(getLegTiming(props.leg).setoff)}</div></div>
      ...
      <div class="row"><div class="label">GPX file</div><div class="value">${asLink(props.gpx_url, "Download")}</div></div>
    `;
    // Hook up sign-up link (opens modal)
    const signupLink = detailsGrid.querySelector(".signupLink");
    if (signupLink) {
      signupLink.addEventListener("click", (e) => {
        e.preventDefault();
        const legKey = signupLink.getAttribute("data-leg");
        if (legKey) openSignupModal(legKey);
      });
    }
  }
  ...
  async function load() {
    ...
    // Populate dropdown with ordered legs
